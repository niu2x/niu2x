...
// the clock sequence multiplexed with the variant. clock_seq_low unsigned small
// 9	      The low field of the clock sequence. node character	      10 -
    ...
    uuid operator()()
    {
        ...
        if (FAILED(hr)) {
            throw std::system_error(
                hr, std::system_category(), "CoCreateGuid failed");
        }

        std::array<uint8_t, 16> bytes = { { static_cast<unsigned char>(
                                                (newId.Data1 >> 24) & 0xFF),
        ...
        std::array<uint8_t, 16> arrbytes = { { bytes.byte0, bytes.byte1,
            bytes.byte2, bytes.byte3, bytes.byte4, bytes.byte5, bytes.byte6,
            bytes.byte7, bytes.byte8, bytes.byte9, bytes.byte10, bytes.byte11,
...
class uuid_time_generator {
    ...
    {
        auto start
            = std::chrono::system_clock::from_time_t(time_t(-12219292800));
