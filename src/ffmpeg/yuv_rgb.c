...
// * G = (Y' - (Rf*R + Bf*B)) / Gf
// From above:
// * G = (Y' - Rf * ((Cr-128)*(255*CrNorm)/CrRange + Y') - Bf *
// ((Cb-128)*(255*CbNorm)/CbRange + Y')) / Gf Since 1-Rf-Bf=Gf, we can take Y'
// out of the division by Gf, and we get:
// * G = Y' - (Cr-128)*Rf/Gf*(255*CrNorm)/CrRange -
// (Cb-128)*Bf/Gf*(255*CbNorm)/CbRange That we can compute, with fixed point
// arithmetic, by
// * G = Y' - ((Cr-128)*[Rf/Gf*(255*CrNorm)/CrRange] +
// (Cb-128)*[Bf/Gf*(255*CbNorm)/CbRange])>>N
//
...
#define YUV2RGB_PARAM(Rf, Bf, YMin, YMax, CbCrRange)                           \
    {                                                                          \
        .cb_factor = FIXED_POINT_VALUE(                                        \
            255.0 * (2.0 * (1 - Bf)) / CbCrRange, 6),                          \
        .cr_factor                                                             \
...
void rgb24_yuv420_std(uint32_t width, uint32_t height, const uint8_t* RGB,
    uint32_t RGB_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
    ...
    for (y = 0; y < (height - 1); y += 2) {
        ...
        uint8_t *y_ptr1 = Y + y * Y_stride, *y_ptr2 = Y + (y + 1) * Y_stride,
        ...
        for (x = 0; x < (width - 1); x += 2) {
            ...
            y_tmp
                = (param->r_factor * rgb_ptr1[0] + param->g_factor * rgb_ptr1[1]
                      + param->b_factor * rgb_ptr1[2])
                >> 8;
            ...
            y_tmp
                = (param->r_factor * rgb_ptr1[3] + param->g_factor * rgb_ptr1[4]
                      + param->b_factor * rgb_ptr1[5])
                >> 8;
            ...
            y_tmp
                = (param->r_factor * rgb_ptr2[0] + param->g_factor * rgb_ptr2[1]
                      + param->b_factor * rgb_ptr2[2])
                >> 8;
            ...
            y_tmp
                = (param->r_factor * rgb_ptr2[3] + param->g_factor * rgb_ptr2[4]
                      + param->b_factor * rgb_ptr2[5])
                >> 8;
...
void rgb32_yuv420_std(uint32_t width, uint32_t height, const uint8_t* RGBA,
    uint32_t RGBA_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
    const RGB2YUVParam* const param = &(RGB2YUV[yuv_type]);
    ...
    for (y = 0; y < (height - 1); y += 2) {
        ...
        for (x = 0; x < (width - 1); x += 2) {
            ...
            y_tmp
                = (param->r_factor * rgb_ptr1[4] + param->g_factor * rgb_ptr1[5]
                      + param->b_factor * rgb_ptr1[6])
                >> 8;
            ...
            y_tmp
                = (param->r_factor * rgb_ptr2[4] + param->g_factor * rgb_ptr2[5]
                      + param->b_factor * rgb_ptr2[6])
                >> 8;
...
void yuv420_rgb24_std(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* U, const uint8_t* V, uint32_t Y_stride, uint32_t UV_stride,
    uint8_t* RGB, uint32_t RGB_stride, YCbCrType yuv_type)
{
    ...
    for (y = 0; y < (height - 1); y += 2) {
        ...
        for (x = 0; x < (width - 1); x += 2) {
            ...
            g_cbcr_offset
                = (param->g_cb_factor * u_tmp + param->g_cr_factor * v_tmp)
                >> 7;
...
void nv12_rgb24_std(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
...
void nv21_rgb24_std(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
    uint32_t RGB_stride, YCbCrType yuv_type)
{
    const YUV2RGBParam* const param = &(YUV2RGB[yuv_type]);
    ...
    // see rgb.txt
    #define UNPACK_RGB24_32_STEP(                                              \
        RS1, RS2, RS3, RS4, RS5, RS6, RD1, RD2, RD3, RD4, RD5, RD6)            \
    ...
    #define RGB2YUV_16(R, G, B, Y, U, V)                                       \
        ...
        Y = _mm_add_epi16(                                                     \
            Y, _mm_mullo_epi16(B, _mm_set1_epi16(param->b_factor)));           \
        Y = _mm_srli_epi16(Y, 8);                                              \
        U = _mm_mullo_epi16(                                                   \
            _mm_sub_epi16(B, Y), _mm_set1_epi16(param->cb_factor));            \
        ...
        Y = _mm_add_epi16(                                                     \
            _mm_srli_epi16(                                                    \
                _mm_mullo_epi16(Y, _mm_set1_epi16(param->y_factor)), 7),       \
            _mm_set1_epi16(param->y_offset));
    ...
    #define RGB2YUV_32                                                         \
        ...
        /* see rgb.txt to get an idea of the algorithm, note that we only go   \
         * to the next to last step*/                                          \
        /* here, because averaging in horizontal direction is easier like      \
         * this*/                                                              \
        /* The last step is applied further on the Y channel only*/            \
        UNPACK_RGB24_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, tmp1, tmp2,   \
            tmp3, tmp4, tmp5, tmp6)                                            \
        UNPACK_RGB24_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, rgb1, rgb2,   \
            rgb3, rgb4, rgb5, rgb6)                                            \
        UNPACK_RGB24_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, tmp1, tmp2,   \
            tmp3, tmp4, tmp5, tmp6)                                            \
        UNPACK_RGB24_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, rgb1, rgb2,   \
            rgb3, rgb4, rgb5, rgb6)                                            \
        /* first compute Y', (B-Y') and (R-Y'), in 16bits values, for the      \
         * first line */                                                       \
        /* Y is saved for each pixel, while only sums of (B-Y') and (R-Y') for \
         * pairs of adjacents pixels are saved*/                               \
        ...
        y1_16 = _mm_add_epi16(                                                 \
            _mm_mullo_epi16(r_16, _mm_set1_epi16(param->r_factor)),            \
            _mm_mullo_epi16(g_16, _mm_set1_epi16(param->g_factor)));           \
        y1_16 = _mm_add_epi16(                                                 \
            y1_16, _mm_mullo_epi16(b_16, _mm_set1_epi16(param->b_factor)));    \
        ...
        y2_16 = _mm_add_epi16(                                                 \
            ...
            _mm_mullo_epi16(g_16, _mm_set1_epi16(param->g_factor)));           \
        y2_16 = _mm_add_epi16(                                                 \
            y2_16, _mm_mullo_epi16(b_16, _mm_set1_epi16(param->b_factor)));    \
        ...
        y1_16 = _mm_add_epi16(                                                 \
            _mm_srli_epi16(                                                    \
                _mm_mullo_epi16(y1_16, _mm_set1_epi16(param->y_factor)), 7),   \
            _mm_set1_epi16(param->y_offset));                                  \
        y2_16 = _mm_add_epi16(                                                 \
            _mm_srli_epi16(                                                    \
                _mm_mullo_epi16(y2_16, _mm_set1_epi16(param->y_factor)), 7),   \
            _mm_set1_epi16(param->y_offset));                                  \
        Y = _mm_packus_epi16(y1_16, y2_16);                                    \
        Y = _mm_unpackhi_epi8(_mm_slli_si128(Y, 8), Y);                        \
        SAVE_SI128((__m128i*)(y_ptr1), Y);                                     \
        /* same for the second line, compute Y', (B-Y') and (R-Y'), in 16bits  \
         * values */                                                           \
        /* Y is saved for each pixel, while only sums of (B-Y') and (R-Y') for \
         * pairs of adjacents pixels are added to the previous values*/        \
        ...
        cb1_16 = _mm_add_epi16(                                                \
            _mm_srai_epi16(_mm_mullo_epi16(_mm_srai_epi16(cb1_16, 2),          \
                               _mm_set1_epi16(param->cb_factor)),              \
                8),                                                            \
            _mm_set1_epi16(128));                                              \
        cr1_16 = _mm_add_epi16(                                                \
            _mm_srai_epi16(_mm_mullo_epi16(_mm_srai_epi16(cr1_16, 2),          \
                               _mm_set1_epi16(param->cr_factor)),              \
                8),                                                            \
            _mm_set1_epi16(128));                                              \
                                                                               \
        ...
        /* Rescale Cb and Cr to their final range */                           \
        cb2_16 = _mm_add_epi16(                                                \
            _mm_srai_epi16(_mm_mullo_epi16(_mm_srai_epi16(cb2_16, 2),          \
                               _mm_set1_epi16(param->cb_factor)),              \
                8),                                                            \
            _mm_set1_epi16(128));                                              \
        cr2_16 = _mm_add_epi16(                                                \
            _mm_srai_epi16(_mm_mullo_epi16(_mm_srai_epi16(cr2_16, 2),          \
                               _mm_set1_epi16(param->cr_factor)),              \
                8),                                                            \
            _mm_set1_epi16(128));                                              \
        /* Pack and save Cb Cr */                                              \
...
void rgb24_yuv420_sse(uint32_t width, uint32_t height, const uint8_t* RGB,
    uint32_t RGB_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
...
void rgb24_yuv420_sseu(uint32_t width, uint32_t height, const uint8_t* RGB,
    uint32_t RGB_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
    ...
    // see rgba.txt
    #define UNPACK_RGB32_32_STEP(RS1, RS2, RS3, RS4, RS5, RS6, RS7, RS8, RD1,  \
    ...
    #define RGBA2YUV_32                                                        \
         ...
         * this*/                                                              \
        /* The last step is applied further on the Y channel only*/            \
        UNPACK_RGB32_32_STEP(rgb1, rgb2, rgb3, rgb4, rgb5, rgb6, rgb7, rgb8,   \
            tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8)                    \
        UNPACK_RGB32_32_STEP(tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8,   \
...
void rgb32_yuv420_sse(uint32_t width, uint32_t height, const uint8_t* RGBA,
    uint32_t RGBA_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_load_si128
...
void rgb32_yuv420_sseu(uint32_t width, uint32_t height, const uint8_t* RGBA,
    uint32_t RGBA_stride, uint8_t* Y, uint8_t* U, uint8_t* V, uint32_t Y_stride,
    uint32_t UV_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_loadu_si128
    ...
    #define UV2RGB_16(U, V, R1, G1, B1, R2, G2, B2)                            \
        r_tmp = _mm_srai_epi16(                                                \
            _mm_mullo_epi16(V, _mm_set1_epi16(param->cr_factor)), 6);          \
        g_tmp = _mm_srai_epi16(                                                \
            _mm_add_epi16(                                                     \
                _mm_mullo_epi16(U, _mm_set1_epi16(param->g_cb_factor)),        \
                _mm_mullo_epi16(V, _mm_set1_epi16(param->g_cr_factor))),       \
            7);                                                                \
        b_tmp = _mm_srai_epi16(                                                \
            _mm_mullo_epi16(U, _mm_set1_epi16(param->cb_factor)), 6);          \
    ...
    #define ADD_Y2RGB_16(Y1, Y2, R1, G1, B1, R2, G2, B2)                       \
        Y1 = _mm_srli_epi16(                                                   \
            _mm_mullo_epi16(Y1, _mm_set1_epi16(param->y_factor)), 7);          \
        Y2 = _mm_srli_epi16(                                                   \
            _mm_mullo_epi16(Y2, _mm_set1_epi16(param->y_factor)), 7);          \
                                                                               \
    ...
    #define PACK_RGB24_32_STEP(                                                \
        RS1, RS2, RS3, RS4, RS5, RS6, RD1, RD2, RD3, RD4, RD5, RD6)            \
        RD1 = _mm_packus_epi16(_mm_and_si128(RS1, _mm_set1_epi16(0xFF)),       \
            _mm_and_si128(RS2, _mm_set1_epi16(0xFF)));                         \
        RD2 = _mm_packus_epi16(_mm_and_si128(RS3, _mm_set1_epi16(0xFF)),       \
            _mm_and_si128(RS4, _mm_set1_epi16(0xFF)));                         \
        RD3 = _mm_packus_epi16(_mm_and_si128(RS5, _mm_set1_epi16(0xFF)),       \
            _mm_and_si128(RS6, _mm_set1_epi16(0xFF)));                         \
        RD4 = _mm_packus_epi16(                                                \
            _mm_srli_epi16(RS1, 8), _mm_srli_epi16(RS2, 8));                   \
        RD5 = _mm_packus_epi16(                                                \
            _mm_srli_epi16(RS3, 8), _mm_srli_epi16(RS4, 8));                   \
        RD6 = _mm_packus_epi16(_mm_srli_epi16(RS5, 8), _mm_srli_epi16(RS6, 8));

    #define PACK_RGB24_32(                                                     \
        R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5, RGB6)            \
        PACK_RGB24_32_STEP(                                                    \
            R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5, RGB6)        \
        PACK_RGB24_32_STEP(                                                    \
            RGB1, RGB2, RGB3, RGB4, RGB5, RGB6, R1, R2, G1, G2, B1, B2)        \
        PACK_RGB24_32_STEP(                                                    \
            R1, R2, G1, G2, B1, B2, RGB1, RGB2, RGB3, RGB4, RGB5, RGB6)        \
        PACK_RGB24_32_STEP(                                                    \
            RGB1, RGB2, RGB3, RGB4, RGB5, RGB6, R1, R2, G1, G2, B1, B2)        \
    ...
    #define LOAD_UV_NV12                                                       \
        ...
        __m128i u = _mm_packus_epi16(_mm_and_si128(uv1, _mm_set1_epi16(255)),  \
            _mm_and_si128(uv2, _mm_set1_epi16(255)));                          \
        uv1 = _mm_srli_epi16(uv1, 8);                                          \
        uv2 = _mm_srli_epi16(uv2, 8);                                          \
        __m128i v = _mm_packus_epi16(_mm_and_si128(uv1, _mm_set1_epi16(255)),  \
            _mm_and_si128(uv2, _mm_set1_epi16(255)));
    ...
    #define YUV2RGB_32                                                         \
        ...
        __m128i r_uv_16_1, g_uv_16_1, b_uv_16_1, r_uv_16_2, g_uv_16_2,         \
            b_uv_16_2;                                                         \
        ...
        __m128i v_16 = _mm_srai_epi16(_mm_unpacklo_epi8(v, v), 8);             \
                                                                               \
        UV2RGB_16(u_16, v_16, r_uv_16_1, g_uv_16_1, b_uv_16_1, r_uv_16_2,      \
            g_uv_16_2, b_uv_16_2)                                              \
        r_16_1 = r_uv_16_1;                                                    \
        g_16_1 = g_uv_16_1;                                                    \
        b_16_1 = b_uv_16_1;                                                    \
        ...
        y_16_2 = _mm_unpackhi_epi8(y, _mm_setzero_si128());                    \
                                                                               \
        ADD_Y2RGB_16(                                                          \
            y_16_1, y_16_2, r_16_1, g_16_1, b_16_1, r_16_2, g_16_2, b_16_2)    \
                                                                               \
        ...
        __m128i rgb_1, rgb_2, rgb_3, rgb_4, rgb_5, rgb_6;                      \
                                                                               \
        PACK_RGB24_32(r_8_11, r_8_12, g_8_11, g_8_12, b_8_11, b_8_12, rgb_1,   \
            rgb_2, rgb_3, rgb_4, rgb_5, rgb_6)                                 \
        ...
        SAVE_SI128((__m128i*)(rgb_ptr1 + 80), rgb_6);                          \
                                                                               \
        PACK_RGB24_32(r_8_21, r_8_22, g_8_21, g_8_22, b_8_21, b_8_22, rgb_1,   \
            rgb_2, rgb_3, rgb_4, rgb_5, rgb_6)                                 \
...
void nv12_rgb24_sse(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
    uint32_t RGB_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_load_si128
...
void nv12_rgb24_sseu(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
    uint32_t RGB_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_loadu_si128
...
void nv21_rgb24_sse(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
    uint32_t RGB_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_load_si128
...
void nv21_rgb24_sseu(uint32_t width, uint32_t height, const uint8_t* Y,
    const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride, uint8_t* RGB,
    uint32_t RGB_stride, YCbCrType yuv_type)
{
    #define LOAD_SI128 _mm_loadu_si128
