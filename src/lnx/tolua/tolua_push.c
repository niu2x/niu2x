...
TOLUA_API void tolua_pushvalue(lua_State* L, int lo) { lua_pushvalue(L, lo); }
    ...
    if (value == NULL)
        ...
        if (lua_isnil(L, -1)) {
            ...
            *(void**)lua_newuserdata(L, sizeof(void*))
            ...
            if (lua_istable(L, -1)) {
                lua_pushstring(
                    L, type); /* stack: mt ubox[u] super super[mt] type */
            ...
            /*luaL_getmetatable(L,type);             // stack: mt ubox[u] super
             * super[mt] flag mt */
            lua_pushvalue(
                L, -5); /* stack: mt ubox[u] super super[mt] flag mt */
            lua_setmetatable(
                L, -5); /* stack: mt ubox[u] super super[mt] flag */
