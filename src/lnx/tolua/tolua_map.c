...
static void mapinheritance(lua_State* L, const char* name, const char* base)
{
    ...
    if (base && *base)
        ...
        if (lua_getmetatable(
                L, -1)) { /* already has a mt, we don't overwrite it */
            lua_pop(L, 2);
            return;
        }
...
/* force garbage collection to avoid releasing a to-be-collected address */
        ...
        if (lua_rawequal(
                L, -1, -2)) /* check that we are releasing the correct type */
        {
            lua_pushlightuserdata(L, u);
            lua_pushnil(L);
            lua_rawset(L, -5);
...
#ifdef LUA_VERSION_NUM /* lua 5.1 */
    int unused = luaL_loadbuffer(L, B, size, name) || lua_pcall(L, 0, 0, 0);
    (void)unused;
#else
