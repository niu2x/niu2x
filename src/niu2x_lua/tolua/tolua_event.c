...
static void storeatubox(lua_State* L, int lo)
{
    ...
    lua_insert(L, -3);
    lua_settable(L, -3); /* on lua 5.1, we trade the "tolua_peers" lookup for a
                            settable call */
    lua_pop(L, 1);
#else
...
/* Access alternative table */
        ...
        if (!lua_rawequal(L, -1, TOLUA_NOPEER)) {
            lua_pushvalue(L, 2); /* key */
            lua_gettable(L, -2); /* on lua 5.1, we trade the "tolua_peers"
                                    lookup for a gettable call */
            if (!lua_isnil(L, -1))
                return 1;
        }
#else
...
{
    /* copying code from do_operator here to return false when no operator is
     * found */
    if (lua_isuserdata(L, 1)) {
        /* Try metatables */
        lua_pushvalue(L, 1); /* stack: op1 op2 */
