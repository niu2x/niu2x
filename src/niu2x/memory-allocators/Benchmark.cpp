...
void Benchmark::SingleAllocation(
    Allocator* allocator, const std::size_t size, const std::size_t alignment)
{
    ...
    BenchmarkResults results = buildResults(
        m_nOperations, std::move(TimeElapsed), allocator->m_peak);

    PrintResults(results);
}

void Benchmark::SingleFree(
    Allocator* allocator, const std::size_t size, const std::size_t alignment)
{
...
void Benchmark::MultipleAllocation(Allocator* allocator,
...
void Benchmark::RandomAllocation(Allocator* allocator,
    const std::vector<std::size_t>& allocationSizes,
    const std::vector<std::size_t>& alignments)
{
    // NOTE: Is this actually initializing the RNG? Jose Fernando Lopez
    // Fernandez 11/07/2018 @ 12:54am (UTC)
    srand(1);
    ...
    while (operations < m_nOperations) {
        this->RandomAllocationAttr(
            allocationSizes, alignments, allocation_size, alignment);
...
{
    // NOTE: Is this actually initializing the RNG? Jose Fernando Lopez
    // Fernandez 11/07/2018 @ 1:51am (UTC)
    srand(1);
...
void Benchmark::PrintResults(const BenchmarkResults& results) const
{
    ...
    std::cout << "\t\tTime elapsed: \t" << results.Milliseconds.count() << " ms"
              << IO::endl;
    std::cout << "\t\tOp per sec:    \t" << results.OperationsPerSec
              << " ops/ms" << IO::endl;
    std::cout << "\t\tTimer per op:  \t" << results.TimePerOperation
              << " ms/ops" << IO::endl;
...
const BenchmarkResults Benchmark::buildResults(std::size_t nOperations,
    std::chrono::milliseconds&& elapsedTime, const std::size_t memoryPeak) const
{
    BenchmarkResults results;

    results.Operations = nOperations;
    results.Milliseconds = std::move(elapsedTime);
    results.OperationsPerSec = results.Operations
