...
//-----------------------------------------------------------------------------
    ...
    // Visual Studio warnings
        ...
        #pragma warning(                                                       \
            disable : 4251) // class 'xxx' needs to have dll-interface to be
                            // used by clients of struct 'xxx' // when IMGUI_API
                            // is set to__declspec(dllexport)
        #pragma warning(disable : 26812) // The enum type 'xxx' is unscoped.
    ...
    // Clang/GCC warnings with -Weverything
        ...
        #pragma clang diagnostic ignored                                       \
            "-Wunused-function" // for stb_textedit.h
        #pragma clang diagnostic ignored                                       \
        ...
        #pragma clang diagnostic ignored                                       \
            "-Wimplicit-int-float-conversion" // warning: implicit conversion
                                              ...
                                              // precision
        #pragma clang diagnostic ignored                                       \
            "-Wmissing-noreturn" // warning: function 'xxx' could be declared
...
namespace ImStb {
    ...
    // don't have many of those so the effect is currently negligible, but
    ...
    // specific item.
            ...
            #define IM_DEBUG_BREAK()                                           \
                IM_ASSERT(                                                     \
                    0) // It is expected that you define IM_DEBUG_BREAK() into
...
IMGUI_API int ImTextCountUtf8BytesFromStr(const ImWchar* in_text,
    ...
    // space, to allow user enabling implicit cast operators between ImVec2 and
    // their own types (using IM_VEC2_CLASS_EXTRA etc.) We unfortunately don't
    // have a unary- operator for ImVec2 because this would needs to be defined
...
IMGUI_API ImU64 ImFileWrite(
    ...
    #else
        #define IMGUI_DISABLE_TTY_FUNCTIONS // Can't use stdout, fflush if we
                                            // are not using default file
                                            // functions
    #endif
...
// Helpers: Maths
IM_MSVC_RUNTIME_CHECKS_OFF
    // - Wrapper for standard libs functions. (Note that imgui_demo.cpp does
    // _not_ use them to keep the code easy to copy)
...
struct IMGUI_API ImChunkStream {
    ...
    // ImDrawList: Helper function to calculate a circle's segment count given
    // its radius and a "maximum error" value. Estimation of number of circle
    // segment based on error is derived using method described in
    ...
    // Raw equation from IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC rewritten for 'r'
    // and 'error'.
    ...
    // ImDrawList: Lookup table size for adaptive arc drawing, cover full
    // circle.
    ...
    #define IM_DRAWLIST_ARCFAST_SAMPLE_MAX                                     \
        IM_DRAWLIST_ARCFAST_TABLE_SIZE // Sample index _PathArcToFastEx() for
                                       // 360 angle.
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_Tabbing = 1 << 9, // == Focus + Activate if item is
                                        // Inputable + DontChangeNavHighlight
    ImGuiNavMoveFlags_Activate = 1 << 10,
    ImGuiNavMoveFlags_DontSetNavHighlight = 1
        << 11 // Do not alter the visible state of keyboard vs mouse nav
              // highlight
};
...
IMGUI_API bool ScrollbarEx(const ImRect& bb, ImGuiID id, ImGuiAxis axis,
    float* p_scroll_v, float avail_v, float contents_v, ImDrawFlags flags);
...
extern const char* ImGuiTestEngine_FindItemDebugLabel(
    ImGuiContext* ctx, ImGuiID id);
        ...
        #define IMGUI_TEST_ENGINE_ITEM_ADD(_BB, _ID)                           \
            ...
            ImGuiTestEngineHook_ItemAdd(                                       \
                &g, _BB, _ID) // Register item bounding box
