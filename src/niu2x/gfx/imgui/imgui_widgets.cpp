    ...
    // Clang/GCC warnings with -Weverything
        ...
        #pragma clang diagnostic ignored                                       \
            "-Wdouble-promotion" // warning: implicit conversion from
                                 ...
                                 // double.
        #pragma clang diagnostic ignored                                       \
            "-Wenum-enum-conversion" // warning: bitwise operation between
                                     // different enumeration types ('XXXFlags_'
                                     // and 'XXXFlagsPrivate_')
        #pragma clang diagnostic ignored                                       \
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    RenderTextClipped(value_bb.Min + style.FramePadding, value_bb.Max,
        value_text_begin, value_text_end, &value_size, ImVec2(0.0f, 0.0f));
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    RenderBullet(window->DrawList,
        bb.Min
            + ImVec2(
                style.FramePadding.x + g.FontSize * 0.5f, g.FontSize * 0.5f),
        text_col);
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    const float scrollbar_size = window->ScrollbarSizes[axis
        ^ 1]; // (ScrollbarSizes.x = width of Y scrollbar; ScrollbarSizes.y =
    ...
    const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton)) {
        ...
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max,
    ...
    switch (data_type) {
        case ImGuiDataType_S8:
            return DataTypeCompareT<ImS8>(
                (const ImS8*)arg_1, (const ImS8*)arg_2);
        case ImGuiDataType_U8:
            return DataTypeCompareT<ImU8>(
                (const ImU8*)arg_1, (const ImU8*)arg_2);
        case ImGuiDataType_S16:
    ...
    const char* fmt, char* fmt_out, size_t fmt_out_size)
{
    ...
    IM_ASSERT((size_t)(fmt_end - fmt + 1)
        < fmt_out_size); // Format is too long, let us know if this happens to
                         // you!
    while (fmt < fmt_end) {
        char c = *(fmt++);
        if (c != '\'' && c != '$'
    ...
    if (is_logarithmic) {
        ...
        float v_old_parametric
            = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
                v_cur, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon,
                zero_deadzone_halfsize);
        float v_new_parametric = v_old_parametric + g.DragCurrentAccum;
        v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
            v_new_parametric, v_min, v_max, is_logarithmic,
    ...
    const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    value_changed
        |= DragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed,
            &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic) {
        ...
        if (t <= 0.0f)
            ...
            if ((v_min * v_max) < 0.0f) // Range crosses zero, so we have to do
                                        // this in two parts
            {
                ...
                result = (TYPE)(v_min_fudged
                    * ImPow(
                        v_max_fudged / v_min_fudged, (FLOATTYPE)t_with_flip));
        }
    } else {
        // Linear slider
    ...
    ImGuiInputTextCallback callback, void* callback_user_data)
{
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput)
            ...
            if (!io.InputQueueCharacters.contains('\t')) {
                ...
                if (InputTextFilterCharacter(&c, flags, callback,
                        callback_user_data, ImGuiInputSource_Keyboard))
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_wordmove_key_down = is_osx
            ? io.KeyAlt
            : io.KeyCtrl; // OS X style: Text editing cursor movement using Alt
                          // instead of Ctrl
        ...
        const bool is_validate_nav = (IsNavInputTest(ImGuiNavInput_Activate,
                                          ImGuiInputReadMode_Pressed)
                                         && !IsKeyPressedMap(ImGuiKey_Space))
            || IsNavInputTest(ImGuiNavInput_Input, ImGuiInputReadMode_Pressed);
        const bool is_cancel = IsKeyPressedMap(ImGuiKey_Escape)
            ...
            if (const char* clipboard = GetClipboardText()) {
                ...
                for (const char* s = clipboard; *s;) {
                    ...
                    if (c == 0)
                        break;
                    if (!InputTextFilterCharacter(&c, flags, callback,
                            callback_user_data, ImGuiInputSource_Clipboard))
                        continue;
...
static void ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
    ...
    // saturation undefined. With color check: hue/saturation is preserved in
    // one widget. Editing color in one widget would reset hue/saturation in
    // another one. Without color check: common hue/saturation would be
    // displayed in all widgets that have hue/saturation undefined.
    // g.ColorEditLastColor is stored as ImU32 RGB value: this essentially gives
    // us color equality check with reduced precision. Tiny external color
    // changes would not be detected and this check would still pass. This is
    // OK, since we only restore hue/saturation _only_ if they are undefined,
    // therefore this change flipping hue/saturation from undefined to a very
    // tiny value would still be represented in color picker.
    ImGuiContext& g = *GImGui;
    ...
    const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags
        ...
        const int fmt_idx = hide_prefix
            ? 0
            : (flags & ImGuiColorEditFlags_DisplayHSV) ? 2 : 1;
    ...
    ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size = ImMax(bars_width * 1,
        width
            - (alpha_bar ? 2 : 1)
                * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value
                                                            // picking box
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        if (IsItemActive()) {
            ...
            // Greatly reduces hue jitter and reset to 0 when hue == 255 and
    ...
    if (NavMoveRequestButNoResultYet()
        ...
        if (nav_earliest_child->ParentWindow == window
            ...
            NavMoveRequestForward(g.NavMoveDir, g.NavMoveClipDir,
                g.NavMoveFlags, g.NavMoveScrollFlags); // Repeat
        }
    }
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p,
    ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderArrow(window->DrawList,
            pos
                + ImVec2(
                    offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f),
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL
            ...
            tb.y = ta.y
                + ImMax((tb.y - extra) - ta.y,
                    -ref_unit
                        * 8.0f); // triangle is maximum 200 high to limit the
    ...
    const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x,
            shortcut_w, checkmark_w); // Feedback for next frame
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    int shrink_buffer_indexes[3] = { 0,
        sections[0].TabCount + sections[2].TabCount, sections[0].TabCount };
    ...
    if (central_section_is_visible)
        width_excess = ImMax(section_1_w
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    if (has_close_button)
        size.x += g.Style.FramePadding.x
            + (g.Style.ItemInnerSpacing.x
                + g.FontSize); // We use Y intentionally to fit the close button
                               // circle.
    else
